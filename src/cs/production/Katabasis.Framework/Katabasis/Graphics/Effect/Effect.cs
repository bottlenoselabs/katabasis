...
namespace Katabasis
{
	...
	public unsafe class Effect : GraphicsResource
	{
		...
		private readonly Dictionary<IntPtr, EffectParameter> _samplerMap = new(new IntPtrBoxlessComparer());
		...
		private void INTERNAL_parseEffectStruct(IntPtr effectData)
		{
			...
			List<EffectParameter> parameters = new();
			for (var i = 0; i < effectPtr->param_count; i += 1)
			{
				    ...
				    param.value.type.parameter_type <= MOJOSHADER_symbolType.MOJOSHADER_SYMTYPE_SAMPLERCUBE)
				{
					...
					{
						if (!string.IsNullOrEmpty(textureName) && textureName.Equals(parameters[j].Name, StringComparison.Ordinal))
						{
			...
			List<EffectTechnique> techniques = new(effectPtr->technique_count);
			...
			uint numAnnotations)
		{
			...
			List<EffectAnnotation> annotations = new((int)numAnnotations);
