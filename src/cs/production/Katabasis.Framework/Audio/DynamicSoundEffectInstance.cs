...
namespace bottlenoselabs.Katabasis
{
	...
	public sealed unsafe class DynamicSoundEffectInstance : SoundEffectInstance
	{
		...
		public void SubmitBuffer(byte[] buffer, int offset, int count)
		{
			...
			lock (_queuedBuffers)
			{
				...
				if (State != SoundState.Stopped)
				{
					...
					FAudioSourceVoice_SubmitSourceBuffer((FAudioSourceVoice*)_handle, (FAudioBuffer*)Unsafe.AsPointer(ref buf), (FAudioBufferWMA*)IntPtr.Zero);
				}
				else
				{
		...
		{
			var buffer = default(FAudioBuffer);
			lock (_queuedBuffers)
			{
				for (var i = 0; i < _queuedBuffers.Count; i += 1)
				{
					...
					FAudioSourceVoice_SubmitSourceBuffer((FAudioSourceVoice*)_handle, (FAudioBuffer*)Unsafe.AsPointer(ref buffer), (FAudioBufferWMA*)IntPtr.Zero);
				}
		...
		internal void Update()
		{
			...
			if (_handle != IntPtr.Zero)
			{
				...
				FAudioSourceVoice_GetState((FAudioSourceVoice*)_handle, &state, FAUDIO_VOICE_NOSAMPLESPLAYED);
