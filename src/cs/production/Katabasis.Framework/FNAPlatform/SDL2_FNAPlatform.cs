...
#pragma warning restore SA1310
        ...
        */
        ...
        {
            GamePadType.Unknown, GamePadType.GamePad, GamePadType.Wheel, GamePadType.ArcadeStick,
            GamePadType.FlightStick, GamePadType.DancePad,
        ...
        public static string ProgramInit(LaunchParameters args)
        {
            ...
            /* We need to change the Windows default here, as the
              ...
              * -flibit
              */
            if (_osVersion.Equals("Windows") || _osVersion.Equals("WinRT"))
            {
            ...
            while (SDL_PeepEvents(
                       ...
                       (uint)SDL_EventType.SDL_CONTROLLERDEVICEADDED,
                       (uint)SDL_EventType.SDL_CONTROLLERDEVICEADDED) == 1)
            {
...
#else
					var exeName =
 Path.GetFileNameWithoutExtension(AppDomain.CurrentDomain.FriendlyName).Replace(".vshost", string.Empty);
            ...
            {
                initFlags |= (uint)SDL_WindowFlags.SDL_WINDOW_ALLOW_HIGHDPI;
            }
...
#pragma warning restore SA1312
            ...
            ref string resultDeviceName)
        {
            ...
            if (wantsFullscreen)
            {
                if ((SDL_GetWindowFlags(windowSDL) & (uint)SDL_WindowFlags.SDL_WINDOW_SHOWN) == 0)
                {
                ...
                var apiResultSetWindowFullscreen = SDL_SetWindowFullscreen(
                    windowSDL, (uint)SDL_WindowFlags.SDL_WINDOW_FULLSCREEN_DESKTOP);
        ...
        public static bool GetWindowResizable(IntPtr window) =>
            (SDL_GetWindowFlags((SDL_Window*)window) & (uint)SDL_WindowFlags.SDL_WINDOW_RESIZABLE) != 0;
        ...
        public static bool GetWindowBorderless(IntPtr window) =>
            (SDL_GetWindowFlags((SDL_Window*)window) & (uint)SDL_WindowFlags.SDL_WINDOW_BORDERLESS) != 0;

        public static void SetWindowBorderless(IntPtr window, bool borderless) =>
            SDL_SetWindowBordered((SDL_Window*)window, !borderless);
        ...
        public static bool SupportsOrientationChanges() =>
            ...
            (_osVersion.Equals("iOS", StringComparison.Ordinal) ||
             _osVersion.Equals("Android", StringComparison.Ordinal));
        ...
        public static Keys GetKeyFromScancode(Keys scancode)
        {
            ...
            if (_xnaMap.TryGetValue((int)scancode, out var retVal))
            {
                ...
                if (_keyMap.TryGetValue(sym, out var result))
                {
                    return result;
                }
            ...
            while (SDL_PollEvent(&evt) == 1)
            {
                ...
                switch ((SDL_EventType)evt.type)
                {
                    ...
                    case SDL_EventType.SDL_WINDOWEVENT:
                        switch ((SDL_WindowEventID)evt.window.@event)
                        {
                            // Window Focus
                    ...
                    case SDL_EventType.SDL_DISPLAYEVENT:
                    {
                        ...
                        // Orientation Change
                        if (evt.display.@event == (int)SDL_DisplayEventID.SDL_DISPLAYEVENT_ORIENTATION)
                        {
                    ...
                    case SDL_EventType.SDL_TEXTINPUT when !textInputSuppress:
                    {
                        ...
                        var bytes = MeasureStringLength(evt.text._text);
                        if (bytes > 0)
                        {
